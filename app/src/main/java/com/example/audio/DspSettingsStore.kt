...
/** Small process-local/shared preference bridge between the Compose/ViewModel layer and audio service. */
object DspSettingsStore {
    ...
    fun read(context: Context): Snapshot {
        val raw = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .getString(KEY_SETTINGS, null) ?: return Snapshot.default()
        return try {
    ...
    private fun putCrossover(j: JSONObject, key: String, c: CrossoverSettings) {
        j.put(key, JSONObject().apply {
            put("hpf", c.hpfEnabled)
            put("hpfFreq", c.hpfFrequency)
            put("hpfSlope", c.hpfSlope.rollOffDb)
            put("lpf", c.lpfEnabled)
            put("lpfFreq", c.lpfFrequency)
            put("lpfSlope", c.lpfSlope.rollOffDb)
        })
    }
    ...
    private fun readCrossover(j: JSONObject?): CrossoverSnapshot = if (j == null) {
        ...
        j.optBoolean("hpf", false), j.optDouble("hpfFreq", 80.0).toFloat(), j.optDouble("hpfSlope", 0.0).toFloat(),
        j.optBoolean("lpf", false), j.optDouble("lpfFreq", 16000.0).toFloat(), j.optDouble("lpfSlope", 0.0).toFloat()
    )

    data class CrossoverSnapshot(val hpf: Boolean, val hpfFreq: Float, val hpfSlope: Float, val lpf: Boolean, val lpfFreq: Float, val lpfSlope: Float)
    data class Snapshot(
        val eqL: FloatArray, val eqR: FloatArray,
        val gainL: Float, val gainR: Float, val muteL: Boolean, val muteR: Boolean,
        val phaseL: Boolean, val phaseR: Boolean, val delayL: Float, val delayR: Float,
        val xL: CrossoverSnapshot, val xR: CrossoverSnapshot
    ) {
        companion object { fun default() = Snapshot(FloatArray(31), FloatArray(31), 0f, 0f, false, false, false, false, 0f, 0f,
