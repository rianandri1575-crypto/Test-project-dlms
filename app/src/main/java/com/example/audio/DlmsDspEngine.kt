...
/**
 * Allocation-free stereo PCM16 DSP core. The engine is deliberately independent of
 * WebView/YouTube so it can process any PCM source owned by the app.
 */
class DlmsDspEngine(private val sampleRate: Int = 48_000) {
    ...
    private fun applyChain(x0: Float, eq: Array<Biquad>, hpf: Array<Biquad>, lpf: Array<Biquad>, x: DspSettingsStore.CrossoverSnapshot): Float {
        var x = x0
        for (f in eq) x = f.process(x)
        if (x.hpf) for (f in hpf) x = f.process(x)
        if (x.lpf) for (f in lpf) x = f.process(x)
        return x
    }

    private fun configure(s: DspSettingsStore.Snapshot) {
        for (i in 0 until 31) {
            val freq = ISO_FREQS[i]
            eqL[i].setPeaking(sampleRate.toFloat(), freq, 1f, s.eqL[i])
            eqR[i].setPeaking(sampleRate.toFloat(), freq, 1f, s.eqR[i])
        }
